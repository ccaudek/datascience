...
Assumendo che le osservazioni future e passate siano condizionalmente indipendenti dato $\theta$, l'espressione precedente può essere scritta come

\begin{equation}
p(\tilde{y}, \theta \mid y) = p(\tilde{y} \mid \theta) p(\theta \mid y).
\end{equation}

La distribuzione predittiva a posteriori viene dunque ottenuta integrando rispetto a $\theta$:
...
Sulla base delle informazioni disponibili, possiamo dunque prevedere, con un livello di certezza soggettiva che eccede la soglia di 0.91, che in un futuro campione di 20 soggetti clinici depressi, il numero di pazienti con depressione grave sarà compreso tra 8 e 16.
